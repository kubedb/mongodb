...
func (c *Controller) ensureAppBinding(db *api.MongoDB) (kutil.VerbType, error) {
	...
	if (db.Spec.SSLMode == api.SSLModeRequireSSL || db.Spec.SSLMode == api.SSLModePreferSSL) &&
		...
		certSecret, err := c.Client.CoreV1().Secrets(db.Namespace).Get(context.TODO(), db.MustCertSecretName(api.MongoDBArchiverCert), metav1.GetOptions{})
		if err != nil {
	...
	if caBundle != nil {
		clientPEMSecretName = meta_util.NameWithSuffix(db.MustCertSecretName(api.MongoDBArchiverCert), api.MongoDBPEMSecretSuffix)
	}
