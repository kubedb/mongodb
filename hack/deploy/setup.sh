...
if [ "$SELF_HOSTED" -eq 1 ]; then
  echo "${KUBEDB_SCRIPT}deploy/kubedb.sh | bash -s -- --operator-name=mg-operator $ARGS"
  ${KUBEDB_SCRIPT}deploy/kubedb.sh | bash -s -- --operator-name=mg-operator ${ARGS}
fi
...
if [ "$MINIKUBE" -eq 1 ]; then
  ...
  cat $INSTALLER_ROOT/deploy/psp/mongodb.yaml | $ONESSL envsubst | kubectl apply -f -
  # Following line may give error if DBVersions CRD already not created
  cat $INSTALLER_ROOT/deploy/kubedb-catalog/mongodb.yaml | $ONESSL envsubst | kubectl apply -f - || true
