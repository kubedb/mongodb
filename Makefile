...
DOCKERFILE_PROD  = hack/docker/mg-operator/in.Dockerfile
DOCKERFILE_DBG   = hack/docker/mg-operator/dbg.Dockerfile
...
install:
	@cd ../installer; \
	APPSCODE_ENV=dev KUBEDB_OPERATOR_TAG=$(TAG) KUBEDB_CATALOG=mongo ./deploy/kubedb.sh --operator-name=$(BIN) --docker-registry=$(REGISTRY) --image-pull-secret=$(REGISTRY_SECRET)
...
check-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes "vendor contrib lubbuild" --check -v
