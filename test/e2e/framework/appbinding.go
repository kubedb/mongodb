...
func (f *Framework) CheckAppBindingSpec(meta metav1.ObjectMeta) error {
	...
	if appBinding.Spec.Secret == nil ||
		...
		(appBinding.Spec.ClientConfig.CABundle != nil && appBinding.Spec.Secret.Name != meta_util.NameWithSuffix(mongodb.MustCertSecretName(api.MongoDBArchiverCert), api.MongoDBPEMSecretSuffix)) {
